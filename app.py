...
def predict(values, dic):
    ...
    if len(values) == 8:
        ...
        model = pickle.load(open('MediInsight\models\diabetes_rf_model.pkl', 'rb'))
        values = np.asarray(values2)
        return model.predict(values.reshape(1, -1))[0]

    # breast_cancer
    elif len(values) == 22:
        model = pickle.load(open(r'MediInsight\models\breast_cancer.pkl', 'rb'))
        values = np.asarray(values)
        return model.predict(values.reshape(1, -1))[0]

    # heart disease
    elif len(values) == 13:
        model = pickle.load(open('MediInsight/models/heart_rf_model.pkl', 'rb'))
        values = np.asarray(values)
        return model.predict(values.reshape(1, -1))[0]

    # kidney disease
    elif len(values) == 24:
        model = pickle.load(open('MediInsight\models\kidney_disease.pkl', 'rb'))
        values = np.asarray(values)
        return model.predict(values.reshape(1, -1))[0]

    # liver disease
    elif len(values) == 10:
        model = pickle.load(open('MediInsight\models\liver.pkl', 'rb'))
        values = np.asarray(values)
        return model.predict(values.reshape(1, -1))[0]
...
def malariapredictPage():
    if request.method == 'POST':
        try:
            ...
            print("load_model")
            model = load_model("MediInsight\models\malaria_detect.h5")
            print("predict")
...
def pneumoniapredictPage():
    if request.method == 'POST':
        try:
            ...
            model = load_model("MediInsight\models\pneumonia.h5")
            pred = np.argmax(model.predict(img))
